<img src="data:image/svg+xml;base64," width="35px" alt="wave"/> Hi, my name is **Sidra Amirbux Khonbati**
=====

### Software Engineering Student | Mobile & Backend Developer | AI Integration

---

Hi, I'm Sidra! 🚀 Software Engineering student at **SZABIST University**, passionate about building mobile and backend applications that solve real problems. I like taking an idea from system design all the way to working software — connecting Flutter apps to Node.js APIs and PostgreSQL databases, and now exploring how AI and OCR fit into that pipeline. My tech stack is Flutter, Node.js, Express.js, PostgreSQL, and Java.

- 🌍  I'm based in **Karachi, Pakistan**
- ✉️  You can contact me at **[sidraamirbux@gmail.com](mailto:sidraamirbux@gmail.com)**
- 🧠  I'm learning **Java, DSA, and AI/OCR integration**
- 🤝  I'm open to collaborating on **Flutter, backend, and document-AI projects**
- 🏦  I built **Finova**, a full digital banking & utility-payment system, and an **OCR account-opening system** for automated document processing
- ⚡  I've interned at **HBL**, **Apexcify Technologies**, **Centura**, and **Skills4U**

<br>

## Skills

<p>
  <img width="42" height="42" src="https://skillicons.dev/icons?i=java" alt="Java"/>
  <img width="42" height="42" src="https://skillicons.dev/icons?i=dart" alt="Dart"/>
  <img width="42" height="42" src="https://skillicons.dev/icons?i=flutter" alt="Flutter"/>
  <img width="42" height="42" src="https://skillicons.dev/icons?i=js" alt="JavaScript"/>
  <img width="42" height="42" src="https://skillicons.dev/icons?i=nodejs" alt="Node.js"/>
  <img width="42" height="42" src="https://skillicons.dev/icons?i=express" alt="Express"/>
  <img width="42" height="42" src="https://skillicons.dev/icons?i=postgres" alt="PostgreSQL"/>
  <img width="42" height="42" src="https://skillicons.dev/icons?i=html,css" alt="HTML CSS"/>
  <img width="42" height="42" src="https://skillicons.dev/icons?i=git,github,vscode,docker,postman" alt="Tools"/>
</p>

## Socials

<p>
  <a href="https://github.com/Sidra-Amirbux-Khonbati">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
  </a>
  <a href="https://www.linkedin.com/in/sidra-amirbux-khonbati-992b9833b/">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
  </a>
  <a href="mailto:sidraamirbux@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail"/>
  </a>
</p>

<br>

## Achievements

<p>
  <img src="https://github-readme-stats-fast.vercel.app/api?username=Sidra-Amirbux-Khonbati&show_icons=true&hide_border=true&theme=dark" width="49%" alt="GitHub stats"/>
  <img src="https://streak-stats.demolab.com?user=Sidra-Amirbux-Khonbati&hide_border=true&theme=dark" width="49%" alt="GitHub streak"/>
</p>

<br>

## Pinned

<table>
<tr>
<td width="50%" valign="top">

**🏦 Finova** `Public`

Digital banking & utility-payment system — Flutter mobile app with a Node.js/Express REST API and PostgreSQL backend. Handles accounts, balances, transfers, bill payments, PDF receipts, and email/SMS notifications.

`Flutter` `Node.js` `PostgreSQL`

</td>
<td width="50%" valign="top">

**📄 OCR Account Opening System** `Public`

Document-processing app that automates data extraction from scanned account-opening forms using OCR, auto-populating structured fields into a database.

`Node.js` `Tesseract.js` `PostgreSQL`

</td>
</tr>
</table>

<br>

<div align="center">
<img src="https://komarev.com/ghpvc/?username=Sidra-Amirbux-Khonbati&label=Profile%20Views&style=flat&color=blue"/>
</div>
